...
def _load_trade_df():
    ...
    # Strip suffix from Ticker column: "AAT_daily_processed" → "AAT"
...
def _fetch_names_background(tickers: list):
    ...
    for i in range(0, len(tickers), batch_size):
        ...
        try:
            # Download info for the whole batch at once
            ...
            for t in batch:
                ...
                try:
                    ...
                    # fast_info doesn't have name, fall back to short_name via info
...
def get_stock_data(ticker):
    ...
    try:
        ...
        # ── Merge raw OHLCV so candlestick chart has open/high/low/volume ──
        ...
        ohlcv_cols = [c for c in ["date", "close", "open", "high", "low", "volume"] if c in df.columns]
        candles = []
...
def search_tickers():
    """Search by ticker symbol OR company name (fuzzy word match)."""
...
def get_predictions(ticker):
    """Return LSTM trade signals for a given ticker from the trade summary CSV."""
    df = _load_trade_df()
    if df.empty:
        return jsonify({"signals": [], "summary": {}})

    # Match by ticker (case-insensitive, handled for _daily_processed suffix)
    # Check for direct match or match with suffix
    ...
    # Load price data to get the price on BuyDate
    buy_price = "N/A"
    try:
...
if __name__ == "__main__":
    ...
    # Kick off background fetch of any missing names
    ...
    print("=" * 60)
    print("  Stock Dashboard →  http://localhost:5050")
    print("=" * 60)
